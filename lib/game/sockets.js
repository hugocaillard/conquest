...
var sockets = {
    ...
    self.io.on('connection', function(socket) {
      if (socket.request.user) {
        ...
        if (owner) {
          ...
          socket.on('upDmg', function(d) {
            game.upDmgTurret.call(this, owner.team)
          });
          socket.on('upLife', function(d) {
            game.upLifeTurret.call(this, owner.team)
          });
          socket.on('disconnect', function() {
            game.setTurret(username, owner, null);
          });
          if (owner.position) var p = mapper.map[owner.position].name;
          else  var p = false;
          socket.emit('joined', {owner: owner, position: p});
          socket.emit('update', owner.turret);
        }
        else
  ...
  sendPositionToTurret: function(position, canSpawn, turret) {
    if (turret) {
      var socket = sockets.io.sockets.connected[turret.socketID];
      if (socket) {
        socket.emit('position', {position: position, canSpawn: canSpawn});
      }
    }
  },
  updateGoldTurret: function(turret, gold) {
    if (turret) {
      var socket = sockets.io.sockets.connected[turret.socketID];
      if (socket) {
        socket.emit('gold', {gold: gold});
      }
    }
  },
