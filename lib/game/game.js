    ...
    for (var tile in self.board) {
      ...
      */
      if (pts && pts.capt) {
          ...
          if (doCompute) {
            ...
            if (tile.scores[team] >= 100) {
              ...
              for (var team in tile.players) {
                for (var i in tile.players[team]) {
                  player = self[team].players[tile.players[team][i]];
        ...
        */
        var username = null, delta = 0;
        for (var team in tile.players) {
          for (var i in tile.players[team]) {
            username = tile.players[team][i];
  ...
  */
  ...
  getTeamsPoints: function(tile) {
    ...
    for (var team in tile.players) {
      var enemies = [];
      if (team === 'alpha')
        enemies = enemies.concat(tile.players.beta, tile.players.gamma);
      else if (team === 'beta')
        enemies = enemies.concat(tile.players.alpha, tile.players.gamma);
      else if (team === 'gamma')
        enemies = enemies.concat(tile.players.alpha, tile.players.beta);

      for (var player in tile.players[team]) {
        player = self[team].players[tile.players[team]];
        ...
        if (player.faction) {
          ...
          // We first compute all the damages
          if (enemies.length)
            self.applyDmg(tile, team, player, enemies)

          // Then we save the healing points to apply it later
  ...
  applyDmg: function(tile, team, player, enemies) {
    ...
    enemy.life -= player.dmg;
    if (enemy.life < 0) {
      player.xp += player.dmg + enemy.life;
      enemy.life = 0;
    }
    else
  ...
  */
  ...
  findOrJoinTeam: function(username) {
    ...
    var player = {
      ...
      faction: null,
      factions: {
        soldier : {xp:0, maxLife:50 ,life:50 ,dmg:5,capt:2, heal:1},
        engineer: {xp:0, maxLife:100,life:100,dmg:2,capt:20,heal:0},
        medic   : {xp:0, maxLife:70 ,life:70 ,dmg:1,capt:1, heal:5}
      }
    };
