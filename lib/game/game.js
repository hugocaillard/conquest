...
var game = {
  ...
  ticker: function() {
    ...
    (function tick() {
      ...
      if (self.alpha.score >= conf.win ||
        ...
        if (self.alpha.score === winner) winner = 'alpha';
        else if (self.beta.score === winner) winner = 'beta';
        else if (self.gamma.score === winner) winner = 'gamma';
        game.sayYo();
        ...
        // save the map in DB every minute
  ...
  */
  ...
  saveMapState: function(winner) {
    var self = this;
    co(function *() {
      var insertedGame = yield db.games.findAndModify({ _id: self.id }, { $set: {
        ticks: self.ticks,
        board: self.board,
        winner: winner,
        teams: {
