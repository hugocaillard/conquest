...
var game = {
  ...
  */
  ticker: function() {
    ...
    (function tick() {
      ...
      if (self.alpha.score >= conf.win ||
        ...
        // update the map in DB every minute
        if (self.ticks%((5*60)) === 0)
  ...
  computePoints: function() {
    ...
    // Check if a team got all the center (the 7 tiles)
  ...
  /**
    * CAPTURE, DAMAGES & HEALING METHODS
  */
  tileCanBeCapture: function(tile, team) {
    // spawns can never be captured
    if (tile.spawn)
      return false;
  ...
  */
  ...
  upgradeSkill: function(d, team) {
    ...
    if (faction && faction.skillPts) {
      if (d.skill == 'maxLife') {
        ...
        faction[d.skill] += 1;
        if (player.faction == 'soldier'  && d.skill == 'dmg'
         || player.faction == 'engineer' && d.skill == 'capt'
         || player.faction === 'medic'   && d.skill == 'heal') {
          faction[d.skill] += 1;
        }
        faction.skillPts--;
      }
  ...
  findOrJoinTeam: function(username, email, bitly) {
    ...
    // If the player is joining for the 1st time
    var player = {
      team: null,
      position: null,
      faction: null,
      ...
      factions: {
        ...
        engineer: {level:1,skillPts:1,xp:0, maxLife:100,life:100,dmg:0,capt:2,heal:0},
        medic   : {level:1,skillPts:1,xp:0, maxLife:70 ,life:70 ,dmg:0,capt:0, heal:4}
      }
    };
    ...
    if (nbInAlpha+nbInBeta+nbInGamma < conf.maxPlayers) {
